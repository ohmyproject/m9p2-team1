...
async function saveRoadmapImage() {
    if (!lastScores || !selectedJob) {
        alert("저장할 데이터가 없사옵니다. 분석을 먼저 진행해 주시게.");
        return;
    }
    ...
    // ── 2행: RIASEC 점수 패널 ──
    const scorePanel = buildScorePanel(lastScores);
    wrap.appendChild(scorePanel);

    // ── 3행: 직무 정보 패널 ──
    const jobPanel = buildJobPanel(selectedJob);
    wrap.appendChild(jobPanel);

    // ── 4행: 로드맵 슬라이드 가로 배치 패널 ──
    const roadmapPanel = buildRoadmapPanel();
    wrap.appendChild(roadmapPanel);
    ...
    // ── html2canvas로 캡처 ──
    try {
        const btn = document.querySelector('.nes-btn.is-primary[onclick="saveRoadmapImage()"]');
