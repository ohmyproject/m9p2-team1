...
def extract_scores_from_pdf(pdf_bytes):
    ...
    m = re.search(
        ...
        text, re.S)
    if m:
        raw = list(map(int, m.group(1).split()))[:6]
        std = list(map(int, m.group(2).split()))[:6]
        if len(raw) == 6 and len(std) == 6:
            return {l: {"원점수": r, "표준점수": s} for l, r, s in zip(labels, raw, std)}

    # 전략 2: 원점수/표준점수 행만 찾기
    m2 = re.search(r"원\s*점\s*수\s*([\d\s]+?)\s*표준\s*점수\s*([\d\s]+)", text, re.S)
    if m2:
        raw = list(map(int, m2.group(1).split()))[:6]
        std = list(map(int, m2.group(2).split()))[:6]
        if len(raw) == 6 and len(std) == 6:
            return {l: {"원점수": r, "표준점수": s} for l, r, s in zip(labels, raw, std)}
    ...
    result = {}
    for label in labels:
        m3 = re.search(rf"{label}[^\d]*(\d+)[^\d]+(\d+)", text)
        if m3:
            result[label] = {"원점수": int(m3.group(1)), "표준점수": int(m3.group(2))}
    if len(result) == 6:
        return result

    # 전략 4: 6+6 숫자 패턴 fallback
    for page_text in full_text:
        numbers = list(map(int, re.findall(r"\b(\d{1,3})\b", page_text)))
        for i in range(len(numbers) - 11):
            g1, g2 = numbers[i:i+6], numbers[i+6:i+12]
            if all(0 <= v <= 50 for v in g1) and all(20 <= v <= 100 for v in g2):
                return {l: {"원점수": r, "표준점수": s} for l, r, s in zip(labels, g1, g2)}

    preview = text[:500].replace("\n", " ")
    raise ValueError(f"PDF에서 점수를 찾을 수 없습니다. 워크넷 직업선호도검사 L형 결과지인지 확인해주세요. (텍스트 미리보기: {preview})")
...
def main():
    ...
    if st.session_state.step == 1:
        ...
        search_query = st.text_input("직무명(잡코리아 기준)을 검색하여 정보 및 로드맵을 확인해보세요")
