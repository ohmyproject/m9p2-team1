...
RAW_SCORE_KEY = "원점수"
T_SCORE_KEY = "표준점수"  # FastAPI index.py와 키 통일
...
PDF_SCORE_PATTERN = re.compile(
    ...
    r"원\s*점\s*수\s*([0-9\s]+?)\s*표준점수\s*([0-9\s]+(?:\s+[0-9]+){5})",
    re.S,
)
...
JOB_FRAME_COLUMNS = [
    "id",
    "대분류",
    "중분류",          # DB의 JK_M_category → normalize_db_job에서 "중분류"로 매핑
    "JK중분류",        # 동일 값, JS/프론트 호환용으로 유지
    "매핑 O*NET 직업명",
    "Top3",
    "참고 유사직업(1차)",
    "전공필수",
    "직무정의",
    "통합_직무정의",
    *list(dict.fromkeys(T_SCORE_COLUMNS.values())),  # 중복 제거 보장
]
...
def get_db_connection():
    try:
        ...
        raise JobCatalogError(
            "pymysql이 설치되어 있지 않습니다. requirements.txt 설치를 확인해주세요."
        ) from exc
...
def normalize_major_required(value) -> str:
...
def normalize_db_job(row: dict) -> dict:
    mid = clean_text(row.get("JK_M_category"))
    job_info = clean_text(row.get("job_information"))

    return {
        "id": row.get("id"),
        "대분류": clean_text(row.get("JK_L_category")),
        "중분류": mid,
        "JK중분류": mid,
        "매핑 O*NET 직업명": clean_text(row.get("similar_job_name")),
        "Top3": clean_text(row.get("top3")),
        "참고 유사직업(1차)": "",
        "전공필수": normalize_major_required(row.get("major_required")),
        "직무정의": job_info,
        "통합_직무정의": job_info,
        "현실형(R) T": safe_float(row.get("realistic_score")),
        "탐구형(I) T": safe_float(row.get("investigative_score")),
        "예술형(A) T": safe_float(row.get("artistic_score")),
        "사회형(S) T": safe_float(row.get("social_score")),
        "진취형(E) T": safe_float(row.get("enterprising_score")),
        "관습형(C) T": safe_float(row.get("conventional_score")),
    }
...
def build_job_tags(row: pd.Series) -> list[str]:
    tags = [
        clean_text(row.get("대분류")),
        clean_text(row.get("중분류")),
        clean_text(row.get("JK중분류")),
    ]
...
def build_job_full_description(row: pd.Series) -> str:
    definition = clean_text(row.get("직무정의")) or clean_text(row.get("통합_직무정의"))
    if definition:
        return definition
    ...
    title = clean_text(row.get("중분류")) or clean_text(row.get("JK중분류"))
    onet_title = clean_text(row.get("매핑 O*NET 직업명"))

    parts = [
        f"{title}는 {category} 분야에서 활동하는 직무입니다.",
        f"O*NET 기준 연관 직무는 {onet_title}입니다." if onet_title else "",
    ]
    if top3:
...
def load_jobs_dataframe(file_path: str | Path | None = None) -> pd.DataFrame:
    if file_path is not None:
        raise JobCatalogError(
            "CSV file_path 로딩은 더 이상 지원하지 않습니다. JK_job DB 테이블을 사용해주세요."
        )
...
def get_job_by_title(title: str) -> dict | None:
    exact_rows = query_job_rows(
        f"{JOB_SELECT_SQL} WHERE JK_M_category = %s ORDER BY id LIMIT 1",
        (title,),
    )
    if exact_rows:
        return serialize_db_job(exact_rows[0])

    partial_rows = query_job_rows(
        f"{JOB_SELECT_SQL} WHERE JK_M_category LIKE %s ORDER BY id LIMIT 1",
        (f"%{title}%",),
    )
    if partial_rows:
