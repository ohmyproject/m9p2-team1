...
function renderRoadmapFromText(rawText) {
    ...
    chatStage.innerHTML = `
        <p class="title">💬 로드맵 상담소</p>
        <p class="roadmap-chat-intro">방금 받은 로드맵과 흥미 점수를 바탕으로 준비 순서, 다른 직무 가능성, 다음 행동을 상담해드릴게요.</p>
        <div class="roadmap-chat-suggestions">
            <button type="button" class="nes-btn is-small" onclick="useRoadmapChatPrompt('내가 가장 먼저 해야 할 일을 알려줘')">첫 행동</button>
            <button type="button" class="nes-btn is-small" onclick="useRoadmapChatPrompt('내 흥미점수 기준으로 다른 직무도 탐색해줘')">다른 직무</button>
            <button type="button" class="nes-btn is-small" onclick="useRoadmapChatPrompt('요즘 이 직무 관련 자격증이나 교육 정보를 확인해줘')">최신 정보</button>
        </div>
        <div id="roadmap-chat-log" class="roadmap-chat-log"></div>
        <div class="roadmap-chat-input-row">
            <input type="text" id="roadmap-chat-input" class="nes-input" placeholder="로드맵이나 직무 준비에 대해 물어보시게..." onkeydown="handleRoadmapChatKey(event)">
            <button type="button" class="nes-btn is-primary" id="roadmap-chat-send" onclick="sendRoadmapChat()">상담</button>
        </div>
    `;
    container.appendChild(chatStage);
    addRoadmapChatMessage("assistant", "로드맵과 흥미 점수를 바탕으로 상담해드릴게요.\n\n준비 순서, 다른 직무 가능성, 다음 행동이 궁금하면 편하게 질문해주세요.");
    updateSlideButtons();
}
...
function addRoadmapChatMessage(role, text, citations = []) {
    ...
    label.textContent = role === 'user' ? '나' : '상담봇';
...
async function sendRoadmapChat(promptOverride) {
    ...
    if (sendBtn) { sendBtn.disabled = true; sendBtn.textContent = '상담 중...'; }
    ...
    try {
        ...
        if (data.status === 'success') {
            ...
            addRoadmapChatMessage('assistant', `상담을 이어가기 어렵사옵니다. ${data.message || ''}`.trim());
        }
    } catch (error) {
        addRoadmapChatMessage('assistant', '서버 연결이 잠시 매끄럽지 않사옵니다. 조금 뒤 다시 물어봐 주시게.');
    } finally {
        if (sendBtn) { sendBtn.disabled = false; sendBtn.textContent = '상담'; }
