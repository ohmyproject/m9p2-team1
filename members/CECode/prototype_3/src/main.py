    ...
    try:
        ...
        if token and chat_context:
            try:
                ...
                save_chat_message(token, chat_context["thread"]["id"], chat_context["session"]["id"], "assistant", result, citations)
                memory_saved = True
            except Exception as e:
                memory_error = str(e)

        response = {"status": "success", "reply": result, "citations": citations, "memory_saved": memory_saved}
        if chat_context:
            response["thread_id"] = chat_context["thread"]["id"]
            response["session_id"] = chat_context["session"]["id"]
            response["roadmap_id"] = chat_context["session"]["roadmap_id"]
        if memory_error:
