...
function updateBackButton() {
    ...
    if (phaseHistory.length === 0 || currentPhaseNum === 1) {
...
function goBackPhase() {
    ...
    const prevPhaseNum = phaseHistory.pop();
    nextPhase(prevPhaseNum, { saveHistory: false });
}

function nextPhase(phaseNum, options = {}) {
    const saveHistory = options.saveHistory !== false;

    if (saveHistory && currentPhaseNum !== phaseNum) {
        phaseHistory.push(currentPhaseNum);
    }
